...
public class RemoveChildActionDTO extends AbstractActionDTO
{
  ...
  protected void buildJson(JsonObject json)
  {
    ...
    json.addProperty(RegistryUrls.GEO_OBJECT_REMOVE_CHILD_PARAM_CHILDCODE, this.childCode);
    json.addProperty(RegistryUrls.GEO_OBJECT_REMOVE_CHILD_PARAM_CHILD_TYPE_CODE, this.childTypeCode);
    json.addProperty(RegistryUrls.GEO_OBJECT_REMOVE_CHILD_PARAM_PARENTCODE, this.parentCode);
    ...
    json.addProperty(RegistryUrls.GEO_OBJECT_REMOVE_CHILD_PARAM_START_DATE, JsonDateUtil.format(startDate));
    json.addProperty(RegistryUrls.GEO_OBJECT_REMOVE_CHILD_PARAM_END_DATE, JsonDateUtil.format(endDate));
  }
  ...
  protected void buildFromJson(JsonObject json)
  {
    ...
    this.childCode = json.get(RegistryUrls.GEO_OBJECT_REMOVE_CHILD_PARAM_CHILDCODE).getAsString();
    this.childTypeCode = json.get(RegistryUrls.GEO_OBJECT_REMOVE_CHILD_PARAM_CHILD_TYPE_CODE).getAsString();
    this.parentCode = json.get(RegistryUrls.GEO_OBJECT_REMOVE_CHILD_PARAM_PARENTCODE).getAsString();
    ...
    this.startDate = JsonDateUtil.parse(json.get(RegistryUrls.GEO_OBJECT_REMOVE_CHILD_PARAM_START_DATE).getAsString());
    this.endDate = JsonDateUtil.parse(json.get(RegistryUrls.GEO_OBJECT_REMOVE_CHILD_PARAM_END_DATE).getAsString());
  }
